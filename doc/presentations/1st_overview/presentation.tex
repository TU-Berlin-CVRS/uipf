...
\begin{frame}{Requirements}
	...
	\begin{itemize}
		\item Plattform independent with linux as primary target
		\item GUI and Command-line mode
		\item Graph based workflow design
		\item No realtime needed
		\item plugin model
		\item logging
...
\begin{frame}{System Overview}
	...
	\begin{tikzpicture}
		...
		\node[mybox] (gui)  at (-4,0) {GUI Interface};
		\node[mybox] (console) at (4,0) {Console Interface};
		\node[mybox,minimum width=10cm] (framework) at (0,-2) {Processing Framework};

		\node[mybox] (config) at (-4,-4) {Configuration Storage};
